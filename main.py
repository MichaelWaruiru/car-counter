...
print("🎥 Video capture initialized:", vid.isOpened())
...
if fps == 0 or fps is None or math.isnan(fps):
    fps = 30.0  # fallback for safe writing
print(f"Video properties -> Width: {width}, Height: {height}, FPS: {fps}")

output_path = "results.mp4"
video_writer = cv.VideoWriter(("result.mp4"), cv.VideoWriter_fourcc(*"mpv4"),
                              fps, (width, height))
print("Video output:", video_writer)
